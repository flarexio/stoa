...
func buildBookEngine(ctx context.Context, kind string, scenario accounting.Scenario, repo accounting.LedgerRepository, amount int64, currency, model string) (llm.ReasoningEngine[usecase.BookkeepingIntent], error) {
	switch kind {
	case "", "scripted":
	...
	case "openai":
		...
		adapter, err := openai.NewAdapter(openai.Config[usecase.BookkeepingIntent]{
			Model:        model,
