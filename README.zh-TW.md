...
目前的切片是一個臨床編碼代理。它讀取臨床紀錄，提出 ICD-10 診斷碼，透過領域規則驗證這些建議，最後只記錄通過驗證的意圖。
...
├── icd/                   # ICD-10 領域模型、驗證器、字典、記錄器
├── coder/                 # 臨床編碼代理迴圈與功能提示詞
├── harness/
│   └── loop/              # 具型別的推理、驗證、執行 runner
├── llm/                   # 共用推理合約與提示詞渲染
│   └── openai/            # OpenAI 供應商轉接器
├── testdata/
│   └── scenarios/         # 確定性場景樣本（例如 tavern.json）
└── docs/
    └── architecture.md
```
...
OpenAI 整合測試會在沒有設定 `OPENAI_API_KEY` 時自動略過：
