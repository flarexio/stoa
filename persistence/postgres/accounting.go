...
// NewRepository returns a Repository that issues every read/write against
// pool. The pool's lifecycle (ConnectPool/Close) belongs to the caller.
...
// accounting.LedgerRepository it backs alongside an io.Closer the caller
// defers to release the pool.
