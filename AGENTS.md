...
- Provider adapter code under `llm/<provider>/` (currently `openai` and `anthropic`) must stay provider-specific: SDK calls, message translation, structured-output selection from `Config.IntentSchema` (e.g. OpenAI `response_format: json_schema`, Anthropic `output_config.format: json_schema`), tool translation, and provider error wrapping only.
