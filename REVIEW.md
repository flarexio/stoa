...
Subdirectory `REVIEW.md` files (e.g. `icd/REVIEW.md`, `llm/openai/REVIEW.md`)
add scoped rules on top of these.
...
- **Wrong-direction imports.** Domain importing `llm/`, `coder/`, or any
  provider SDK. `harness/` importing a provider SDK. `llm/` importing a
...
- **Public contract changes without callers updated.** Touching
  ...
  `harness/loop.Runner` requires the same PR to update `coder/`,
  `llm/openai/`, and tests.
