...
ICD 切片是相同架構套用於臨床編碼的早期驗證。它讀取臨床紀錄，提出 ICD-10 診斷碼，透過領域規則驗證這些建議，最後只記錄通過驗證的意圖。它作為模式範例保留，而非產品方向。
...
重點是 `icd/` 完全不需要知道 AI 是什麼。它擁有醫療編碼的領域模型：病歷紀錄、ICD 代碼建議、字典、記錄器與驗證規則。`coder/` 負責使用案例迴圈和 ICD 專屬提示詞。`llm/openai/` 只是其中一個基礎設施轉接器，用來滿足共用的推理引擎合約。
...
├── accounting/            # 會計領域：帳本、科目、期間、驗證器
├── bookkeeper/            # 記帳代理迴圈與提示詞渲染
├── icd/                   # ICD-10 領域模型、驗證器、字典、記錄器（範例）
├── coder/                 # 臨床編碼代理迴圈與功能提示詞（範例）
├── harness/
