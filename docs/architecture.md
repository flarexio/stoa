...
Example: `accounting/` defines the ledger domain -- `Account`, `Period`, `JournalEntry`, `JournalIntent`, the `Validator`, and the `LedgerRepository` port. `accounting/usecase/` holds the `PostJournal` application operation -- validate-then-publish, callable without an LLM -- together with the transport ports (`EventPublisher`, `EventBus`) it publishes through. `accounting/agent/` is the agent that drives `PostJournal` through the reason-validate-publish loop and renders the bookkeeping prompt. The OpenAI wiring happens at the composition edge, where `accounting/agent`'s prompt renderer is passed into `llm/openai`. `accounting/` imports neither `accounting/usecase/`, `accounting/agent/`, nor `llm/`. The `world/` + `world/agent/` pair is a second feature slice with the same shape, minus the `usecase/` package its simpler flow does not yet need.
