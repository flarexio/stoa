...
Example: `accounting/` defines the ledger domain -- `Account`, `Period`, `JournalEntry`, `JournalIntent`, the `Validator`, and the `LedgerRepository` port. `accounting/usecase/` holds the application operations -- `PostJournal` and `ReverseJournal`, each validate-then-execute and callable without an LLM -- the `BookkeepingIntent` discriminated union and `Registry` that route to them, and the transport ports (`EventPublisher`, `EventBus`) they publish through. `post_journal` carries a `JournalIntent`, while `reverse_journal` carries a `ReverseIntent` that resolves to a `JournalIntent` before validation. `accounting/agent/` is the agent that drives one harness loop over `usecase.BookkeepingIntent`, letting the model pick an intent per turn, and renders the bookkeeping prompt. The OpenAI wiring happens at the composition edge, where `accounting/agent`'s prompt renderer is passed into `llm/openai`. `accounting/` imports neither `accounting/usecase/`, `accounting/agent/`, nor `llm/`. The `world/` + `world/agent/` pair is a second feature slice with the same shape, minus the `usecase/` package its simpler flow does not yet need.
